...
function Nmea0183Instance(trace, traceerrorValue, prefix)
{
    ...
    // Module support
    self.moduleName = "Nmea0183Instance";
    self.trace = false;
    self.traceerrorValue = true;
    ...
    if (typeof(traceerrorValue) != "undefined") {
        self.traceerrorValue = traceerrorValue;
    }
    ...
    //
    self.configuredMaxLength = 80;

    // Includes the <CR><LF>
    self.maxSendLength = self.configuredMaxLength + 2;
    ...
    //
    self.maxLength = self.configuredMaxLength - (self.prefix.length + 3);

    //
    // create a default handler for the streaming
    // API's.
    //
    self.channelWrite = function(data, callback) {
...
//
// Some common NMEA devices today such as GPS units send greater than
// the original specifications 80 maximum character length.
//
// This allows those applications to set a higher maximum.
//
Nmea0183Instance.prototype.setMaxLength = function(length)
{
    var self = this;

    self.configuredMaxLength = length;
...
Nmea0183Instance.prototype.generateSentence = function() {
    ...
    if (sentence.length > this.maxSendLength) throw "generateSentence bad max length";
...
//         nv.value = null
//
Nmea0183Instance.prototype.splitCommandValue = function(str)
{
    var o = new Object();
